                ...
                <div class="px-6 py-6 lg:px-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex justify-between gap-4">
                            <div>
                                <label for="is_enabled" class="block mb-2 text-sm font-medium text-gray-900 ">
                                    ¿Habilitada?
                                </label>
                                <div class="relative text-center">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="is_enabled" v-model="isEnabled" class="sr-only peer">
                                        <div
                            ...
                            <div>
                                <label for="required_admin_permission" class="block mb-2 text-sm font-medium text-gray-900 ">
                                    ¿Requiere Permisos de Admin?
                                </label>
                                <div class="relative text-center">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="required_admin_permission" v-model="requiredAdminPermission"
                                            class="sr-only peer">
                                        <div
                                            class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600">
                                        </div>
...
export default {
    ...
    methods: {
        async submit() {
            try {
                this.family.is_enabled = this.isEnabled ? 1 : 0;
                this.family.required_admin_permission = this.requiredAdminPermission ? 1 : 0;
                this.family.name = this.name;
                const response = await axios.put('/families/' + this.family.id, this.family);
    ...
    computed: {
        isEnabled: {
            set(value) {
                this.family.is_enabled = value ? 1 : 0;
            },
            get() {
                return this.family.is_enabled === 1;
            }
        },
        requiredAdminPermission: {
            set(value) {
                this.family.required_admin_permission = value ? 1 : 0;
            },
            get() {
                return this.family.required_admin_permission === 1;
            }
        }
    }
};
